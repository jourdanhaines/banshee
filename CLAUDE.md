...
- `internal/notify/` — desktop notifications straight over the session bus's `org.freedesktop.Notifications` (godbus), because neither gio (no expire/persist) nor notify-send (blocks to report actions) can express the plugin notification contract. `New` does no I/O — the bus is dialed lazily on first `Send`, redialed once per Send after a failure; `Request.RequireInput` maps to expire 0 + resident hint + critical urgency (unless set), `TimeoutMS` to expire_timeout, `Key` to replaces_id; `OnEvent` callbacks fire on the notifier's signal goroutine — never the GTK loop — and `Probe()` is doctor's health check. `Request.SoundPath` plays an audio file on a successful send: banshee plays it itself (first of `pw-play`/`paplay`/`ffplay` on PATH, spawned detached) and deliberately does **not** set the spec's `sound-file` hint — mako and dunst ignore the hint, and self-play can't double up on a daemon that honors it. Test seams are `Options.Connect`, a func field returning a `Conn` struct of funcs (cliphist `WatcherOptions` precedent, no interface), and `Options.Play`.
...
- **Notifications** — an exec plugin may push a desktop notification at any time by writing an out-of-band, seq-less `notify` message (`WireNotify`: id, summary, per-notification `require_input` / `timeout_ms`, urgency, action buttons, and a `sound` audio file — relative resolves against the plugin dir, ~ expands); the host resolves the icon and sound paths, gen-gates the message like `deliver`, and hands it to `Options.Notify` — the sink boot injects (`boot.pluginNotify`), which gates on the `notifications` conf key (an `atomic.Bool`, flipped by reload, no plugin restart) and maps it onto `internal/notify` keyed `pluginID + "/" + id` so re-sends replace. Invoked actions and closes come back as fire-and-forget `notify-action` / `notify-closed` events with the Submit caveat (a restarted plugin may not know the id). A nil sink — tests, doctor — drops pushes.
- **Background plugins** — `exec.background: true` starts the plugin when the daemon runs and restarts it whenever it exits: `Host.StartBackground()` (called by `boot.run`, like the clipboard watcher — never by `boot.New`) sets a sticky flag so every later `Load` (i.e. reload) re-spawns them too, and `ExecPlugin.superviseLoop` reuses the ordinary crash accounting, so backoff and disable-until-reload behave exactly as for lazy plugins. A background plugin with no `prefix` is excluded from `Providers()` — it exists to push notifications, and an empty prefix would match every keystroke (it must still answer queries with empty `done:true` results, because an old banshee ignores the key and loads it lazily). `plugins/claude-code/` is the shipped one: a FIFO under `$XDG_RUNTIME_DIR/banshee/`, a `hook.sh` users register in Claude Code's hook config (writes the hook JSON plus `$PPID` through `timeout 2`, always exits 0), per-plugin options in a sh-sourced `config` file, and a notification click that walks the recorded pid's `/proc` ancestry to focus the Hyprland client running that session. It notifies only when input is required or work truly finished: `Stop` is out of the default `EVENTS` (fires on every turn end, even mid-subagent-wait) and `Notification` events are filtered by their `notification_type` against `NOTIFY_TYPES` (default `agent_needs_input agent_completed`; `idle_prompt` fires while merely waiting on subagents, `permission_prompt` duplicates `PermissionRequest`); a payload without the field notifies, so older Claude Code versions keep the old behavior.
