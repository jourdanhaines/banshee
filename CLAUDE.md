...
- `internal/providers/totp/` — TOTP codes with a live countdown (`CatTOTP`): RFC 6238 + `otpauth://` parsing, the non-secret `totp.json` index, the `totp`/`otp` trigger, and the `totp-copy` / `totp-add` / `totp-setup` / `totp-wizard-reset` / `totp-wizard-fix` actions. Seeds live in `internal/secrets`, never in the index. A backend-unusable failure arms the shared `SetupState` and boot's `Reopen` hook re-shows the launcher with the setup wizard's guidance/retry rows ("wizard-as-results-rows", `wizard.go`) instead of a dead-end toast; which errors route there lives on `backendUnusable` in `handlers.go`. A `totp-wizard-fix` row runs a user-level repair in-process from the package's own `wizardFixes` table — the action carries `[backend, fixID]` as a lookup key, never argv — and finishes setup itself so a repair that worked retires the wizard without a manual Retry; privileged repairs (the package install) go out as `providers.ActTerminal` so sudo prompts in the user's own terminal and banshee never sees the password, and degrade to the distro-neutral clipboard-copy row when no `packageManagers` binary is on PATH.
