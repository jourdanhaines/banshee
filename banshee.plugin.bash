...
# Bash uses readline for keybindings
_banshee_bind_key() {
    local key_seq
...
_banshee_completions() {
    ...
    # If exactly one match, we're done (normal completion)
    # If multiple matches and user hits tab again, launch banshee with the query
    if [[ ${#COMPREPLY[@]} -gt 1 && -n "$cur" ]]; then
        # Keep the completions for display, user can tab-cycle or hit enter
        return
    fi
}
